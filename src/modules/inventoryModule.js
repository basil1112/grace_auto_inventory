...
exports.newBillAddItem = (data, callback)=>{
    dbModule.insert('temp_bill', data, function(err, result) {
        callback(err, result);
    });
}


exports.getTempBillItems = (data, callback)=>{
    
   return new Promise((resolve,reject) =>{
        dbModule.selectQuery('SELECT b.id,b.itemID,b.receipts,b.unitValue,b.username, p.name as itemname, b.comments,b.datetime FROM temp_bill b , items p where p.id = b.itemID', (err, rows) => {
            if(err) {
                reject(err);
            } else {
               resolve(rows);
...
exports.removeItemFromEstimateBill = (id)=>{
    return new Promise((resolve,reject)=>{
        dbModule.delete('temp_bill',`id=${id}`,(err,data)=>{
            if(data == 'success'){
               resolve(true)
            }
            if(err){
...
exports.deleteAllTempEstimateBill = async (id)=>{
   
        dbModule.deleteAll('temp_bill',(err,data)=>{
            if(data == 'success'){
               //resolve(true)
            }
            if(err){
...
exports.getItemInStock = (itemID) => {

    return new Promise((resolve,reject)=>{
        let query = `SELECT SUM(receipts) as stock FROM "entries" where itemID=${itemID}`
        dbModule.selectQuery(query,(err,rows)=>{
            if(err){
                reject(err);
            }else{
                resolve(rows);
